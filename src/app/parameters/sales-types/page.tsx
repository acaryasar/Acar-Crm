...
export default async function SalesTypesPage({
  ...
  if ((mode === 'edit' || mode === 'view') && salesTypeId) {
    salesType = await prisma.saleType.findUnique({
  ...
  if (mode !== 'list') {
    return <SalesTypeForm mode={mode} salesType={salesType as any || undefined} />;
  }

  return <SalesTypeList salesTypes={salesTypes as any} />;
}
