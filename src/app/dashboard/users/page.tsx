...
import { requireRole, isAdminOrSupervisor, getCompanyIdFilter, isAdmin } from "@/lib/auth-guard";
...
export default async function UsersPage({
      ...
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between mb-6 shrink-0">
          ...
          <div className="flex items-center gap-3">
            {mode === "view" && user && (
              <Link 
                href={`/dashboard/users?mode=edit&id=${user.id}`}
  ...
  // Admin tüm user'leri görebilir, diğer roller sadece kendi user'sini
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-6 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <UserSearch />
          <Link
            href="/dashboard/users?mode=create"
            className="inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-colors shadow-sm min-w-[140px]"
          >
            <Plus size={12} />
            {t("new")}
