  ...
  try {
    ...
    if (!demoSession) {
      ...
      demoSession = {
        ...
        email: `demo-${Math.random().toString(36).substr(2, 9)}@example.com`,
      };
    ...
    console.error("Error in Email demo chat:", error);
...
async function processStep(session: DemoSession, message: string, companyId: string): Promise<string> {
  ...
  switch (session.step) {
    case "greeting":
      ...
      return "Merhaba! Email AI asistanı olarak size yardımcı olmaktan mutluluk duyarım. Lütfen adınızı söyleyin.";
  ...
  try {
    ...
    const ticket = await prisma.ticket.create({
      data: {
        ...
        title: `${session.data.category || "Genel"} - Email Talebi`,
        description: session.data.description || "Email üzerinden gelen talep",
        source: "EMAIL",
        status: "NEW",
        category: "OTHER",
    ...
    if (session.data.appointmentDate) {
      ...
      await prisma.appointment.create({
        data: {
          companyId,
          customerId: customer.id,
          title: "Email Randevusu",
          description: session.data.description || "Email üzerinden gelen randevu",
