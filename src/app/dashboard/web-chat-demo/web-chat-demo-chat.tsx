...
import { Send, Bot, User, RefreshCw, ArrowLeft, MessageCircle } from "lucide-react";
...
interface WebChatDemoChatProps {
  ticketId?: string;
}

export function WebChatDemoChat({ ticketId }: WebChatDemoChatProps) {
  ...
  useEffect(() => {
    if (ticketId) {
      setIsReadonly(true);
      fetchWebChats(ticketId);
    }
  }, [ticketId]);

  const fetchWebChats = async (ticketId: string) => {
    try {
      ...
      if (data.webChatSessions && data.webChatSessions.length > 0) {
        const formattedMessages: Message[] = data.webChatSessions.map((session: any) => ({
          role: 'assistant',
          content: `Web Chat: ${session.chatLog || 'İçerik bulunamadı'}`,
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("/api/ai/web-chat-demo", {
        method: "POST",
        headers: {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-semibold">
              {isReadonly ? "Web Chat Geçmişi" : "Web-Chat AI"}
            </p>
            <p className="text-orange-100 text-xs">
              {isReadonly ? "Sadece görüntüleme modu" : "AI ile web chat"}
            </p>
          </div>
        </div>
        {!isReadonly && (
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-slate-600 font-medium">
              {isReadonly ? "Henüz mesaj yok" : "Web-Chat AI"}
            </p>
            <p className="text-slate-400 text-sm mt-1">
              {isReadonly
                ? "Bu ticket için web chat kaydı bulunmamaktadır."
                : "AI ile Web Chat Asistanı. Web chat benzeri bir arayüz ile AI ile etkileşim kurabilirsiniz."}
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start gap-2">
                  {message.role === "assistant" && (
                    <Bot className="w-4 h-4 text-orange-600 mt-0.5 shrink-0" />
                  )}
                  ...
                  {message.role === "user" && (
                    <User className="w-4 h-4 text-orange-100 mt-0.5 shrink-0" />
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-slate-200 p-4 bg-white">
          <div className="flex items-center gap-2">
            <textarea
              ...
              className="flex-1 resize-none border border-slate-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
              rows={1}
            />
            <button
              onClick={handleSend}
